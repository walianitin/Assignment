        ...
        (data as ApiDataItem[])
          ...
          .forEach((item) => {
            if (!countryMap.has(item.EconomyName)) {
              const parsedValue = parseFloat(item.DatapointValue);
              countryMap.set(item.EconomyName, {
                countryName: item.EconomyName,
                value: isNaN(parsedValue) ? "N/A" : parsedValue.toFixed(2),
              });
            }
          });

        // Then, add missing countries with "N/A" value
        apiCountryNames.forEach((countryName) => {
        ...
        {hasAnyData && Object.entries(filteredData).map(([indicator, data]) => (
          data.length > 0 && (
            <div key={indicator} className="transform hover:-translate-y-1 transition-transform duration-200">
              <ChartBarMixed
                data={data.map((item) => {
                  const numericValue = item.value === "N/A" ? 0 : parseFloat(item.value);
                  return {
